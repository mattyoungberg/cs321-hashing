...
# I used the tee command to direct script results to both stdout and a file due
# to uncertainties in the desired output format.
#
# To clarify, what I mean by "script results" is the output generated by the
# `run-tests.sh` script itself. This output is distinct from the file that's
# generated by the `HashtableTest` program at debugLevel=1.
#
# The unused header function we were given suggested we were meant to append
# headers into a file. However, altering it to work with stdout or changing the
# test script's output format entirely didn't seem in line with the scope of the
# given task. To reconcile these aspects and avoid altering the previous
# behavior too much, I employed the `tee` command. This approach ensured a
# balance, sending test outputs to both stdout and a file.
